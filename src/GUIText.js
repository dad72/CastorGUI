class GUIText extends GUIManager {
	...
	addElement(append, element = null) {
		...
		if(this.inline == false) {
			...
			this.textElement.style.width = GUIText.getTextWidth(this.texte, this.font).w+"px";			
		}
		this.textElement.style.height = GUIText.getTextWidth(this.texte, this.font).h+"px";		
		...
		if(append == true) {
			if(this.centerVertical) {
				let marginTop = ((this.getCanvasSize().height / 2) - (GUIText.getTextWidth(this.texte, this.font).h / 2));
				this.textElement.style.top = (marginTop + this.getCanvasOrigine().top)+signe;
			}
			if(this.centerHorizontal) {
				let marginTotal = (this.getCanvasSize().width - GUIText.getTextWidth(this.texte, this.font).w);
			...
			if(this.centerVertical) {
				this.textElement.style.top = "calc(50% - "+(GUIText.getTextWidth(this.texte, this.font).h / 2)+"px)";
			}
	...
	updateText(texte){
		...
		if(this.append == true) {
			this.textElement.style.width = GUIText.getTextWidth(texte, this.font).w+signe;
			this.textElement.style.height = GUIText.getTextWidth(texte, this.font).h+signe;
			this.textElement.style.top = (this.textPosition.y + this.getCanvasOrigine().top)+signe;
			this.textElement.style.left = (this.textPosition.x + this.getCanvasOrigine().left)+signe;
		} else {
			this.textElement.style.top = this.textPosition.y+signe;
		...
		if(this.centerVertical == "true") {
			let marginTop = ((this.getCanvasSize().height / 2) - (GUIText.getTextWidth(texte, this.font).h / 2));
			this.textElement.style.top = (marginTop + this.getCanvasOrigine().top)+signe;
		}
		if(this.centerHorizontal == "true") {
			let marginTotal = (this.getCanvasSize().width - GUIText.getTextWidth(texte, this.font).w);
			let marginLeft = (marginTotal / 2);
			this.textElement.style.left = (marginLeft + this.getCanvasOrigine().left)+signe;
		}
